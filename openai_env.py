...
class Env:
    ...
    def single_step(self, action: int):
        if action == 1: # exercise opt
            reward = max(self._strike - self._spot, 0.0) * np.exp(-self._r * self._float_time * (self._day_step / self._days))
            done = True
        else: # hold opt
            if self._day_step == self._days: # if at maturity
                reward = max(self._strike - self._spot, 0.0) * np.exp(-self._r * self._time)
                done = True
            else: # check tmr
                reward = 0
                self._spot = self._spot * np.exp((self._rate - 0.5 * self._sigma ** 2) * (self._time / self._days) + self._sigma * np.sqrt(self._time / self._days) * np.random.normal())
                self._day_step += 1
                done = False
